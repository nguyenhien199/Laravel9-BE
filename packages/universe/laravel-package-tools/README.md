...
use Universe\PackageTools\Commands\InstallCommand;
...
class YourPackageServiceProvider extends PackageServiceProvider
{
    public function configurePackage(Package $package): void
    {
        $package
            ...
            ->hasConfigFile()
            ->hasViews()
            ->hasViewComponent('universe', Alert::class)
...
In your package you should let your service provider extend `Universe\PackageTools\PackageServiceProvider`.
...
$package
    ->name('your-package-name')
    ->sharesDataWithAllViews('companyName', 'Universe');
```
...
$package
    ->name('your-package-name')
    ->hasViewComponents('universe', Alert::class);
```

This will register your view components with Laravel. In the case of `Alert::class`, it can be referenced in views
as `<x-universe-alert />`, where `universe` is the prefix you provided during registration.
...
Any translations your package provides, should be placed in the `<package root>/lang/<language-code>`
directory.
...
Assuming you save this translation file at `<package root>/lang/en/translations.php`...
...
in `<package root>/lang/<language-code>.json`.

For example, creating `<package root>/lang/it.json` file like so:
...
```php
use use Universe\PackageTools\Commands\InstallCommand;
